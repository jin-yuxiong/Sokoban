...
State* State::boxPushed(int i, int j, Direction d) {
	...
	if (tiles[ci*width + cj] != Character && tiles[ci*width + cj] != CharacterinAid) {//可以有推的人
		return nullptr;
	}
	if (tiles[i*width + j] != Box && tiles[i*width + j] != BoxinAid) {
		return nullptr;
	}
	if (tiles[newi*width + newj] == Wall || tiles[newi*width + newj] == Box || tiles[newi*width + newj] == BoxinAid) {//可以推到这里
		return nullptr;
	}
	State * res = clone();
